...
class ArrayFacetsComponent(RegularFacetsComponent):
    ...
    def build_facets(self, datatype, facets, facet_definition = None, searchable = True):
        ...
        if hasattr(datatype.item, 'facets'):
            ...
            field = facet_section.get('field', facet_section['facet_class'] + f'(field=\"{path}\", label =_(\"{label}\") {arguments_string} ' + ')')
            facet_definition = {'path' : key, 'class' : field , 'facet_searchable': facet_searchable, 'imports': imports}
