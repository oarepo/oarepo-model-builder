...
class RegularFacetsComponent( DataTypeComponent):
    ...
    def build_facets(self, datatype, facets, facet_definition = None, searchable = True):
        ...
        if not facet_section.config == {}:
            ...
            label = facet_section.config.get('label', f'{_path.replace(".", "/")}.label')
            ...
            field = facet_section.config.get('field', datatype.section_facets.config['facet_class'] + f'(field=\"{path}\", label =_(\"{label}\") {arguments_string} ' + ')')
            facet_definition = {'path' : key, 'class' :  field,
