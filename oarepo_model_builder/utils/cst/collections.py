...
class DictMerger(MergerBase):
    ...
    def merge(self, context: PythonContext, existing_node, new_node):
        ret = []
        existing_elements = {
            el.key.value: el for el in existing_node.elements
        }
        new_elements = {
            el.key.value: el for el in new_node.elements
        }
        for k, el in existing_elements.items():
            if k not in new_elements:
                ret.append(el)
            else:
                ret.append(el.with_changes(
                    value=merge(context, el.value, new_elements.pop(k).value, expression_mergers) or el.value
                ))
