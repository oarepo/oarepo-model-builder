...
class InvenioRecordMetadataAlembicSetupCfgBuilder(OutputBuilder):
    ...
    def finish(self):
        ...
        output.add_entry_point(
            "invenio_db.alembic",
            self.model.record_schema_metadata_alembic,
            f"{self.model.package}:alembic",
        )

        python_path = self.model.package_path / "alembic" / "__init__.py"
    ...
    # {self.model.record_metadata_table_name} and its versioned counterpart, otherwise
    # alembic below will not work !
    invenio db create 
            
    # create the branch
    invenio alembic revision "Create {self.model.record_schema_metadata_alembic} branch."  -b {self.model.record_schema_metadata_alembic} -p dbdbc1b19cf2 --empty
    
    # apply the branch
    invenio alembic upgrade heads
    
    # initial revision
    invenio alembic revision "Initial revision." -b {self.model.record_schema_metadata_alembic}
