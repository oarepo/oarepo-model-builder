...
class InvenioBaseClassPythonBuilder(PythonBuilder):
    ...
    def finish(self, **extra_kwargs):
        super().finish()
        python_path = self.class_to_path(self.model[self.class_config])
        self.process_template(
            python_path,
            self.template,
            current_package_name=package_name(self.model[self.class_config]),
            **extra_kwargs,
        )
    ...
    def process_template(self, python_path, template, **extra_kwargs):
        ...
        context = HyphenMunch(settings=self.settings, model=self.model, **extra_kwargs)
