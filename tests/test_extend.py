...
from oarepo_model_builder.loaders.extend import extend_modify_marshmallow
...
def test_extend_marshmallow():
    ...
    model = ModelSchema(
        ...
        schema_preprocessors=[extend_modify_marshmallow],
    )
    builder.build(model, "record", ["record"], "")

    loaded_model = json5.loads(fs.read("test/models/records.json"))
    # the model should be extended as there is an extra module={qualified: test} section on the model
    ...
    assert "metadata = ma_fields.Nested(lambda: BlahMetadataSchema())" in schema
